...
const playersRanking = [
  { name: `Gabriel`, points: 100 },
  { name: `Natalia`, points: 97 },
];
...
const welcome = () => {
  ...
  console.log(
    `If you complete the card in 15 turn you will have 100 points! For every other turn, you will lose one point`
  );
  return player;
};
...
const likeCard = () => {
  ...
  do {
    ...
    !likedCard ||
    (likedCard.toLowerCase() !== `yes` && likedCard.toLowerCase() !== `no`)
  );
...
const generateCard = () => {
  ...
  const colum1 = numbersCollection.splice(10, 5);
  const colum2 = numbersCollection.splice(5, 5);
  const colum3 = numbersCollection.splice(0, 5);
  bingoCard.push(colum1, colum2, colum3);
  likeCard();
};

const call = () => {
  const numberCall = Math.floor(Math.random() * 20 - 1) + 1;
  ...
  for (let i = 0; i < bingoCard.length; i++) {
    if (bingoCard[i].includes(numberCall)) {
      const indexOfCalledNUmber = bingoCard[i].indexOf(numberCall);
...
const checkForLines = () => {
  for (let i = 0; i < bingoCard.length; i++) {
    const checkIt = bingoCard[i].every((check) => check === `X`);
    if (checkIt) {
      bingoCard[i] = [`LINE!`];
      console.log(`Line!!!!`);
    }
...
const point = (playerName) => {
  ...
  console.log(
    `Youre points are ${playerPoints}! Congratulations ${playerName}`
  );
  playersRanking.push({
    name: playerName,
    points: playerPoints,
...
const bingo = () => {
  const bingoWin = checkForBingo();
  if (bingoWin) {
    return;
  }
  const newRound = window.confirm(`You want to play a new round?`);
  if (newRound) {
    call();
